...
// device stack keeps being serviced while we sit at a prompt.
void (*gPump)() = nullptr;
...
// prompt that parked in getchar() would stall it. This still waits
// indefinitely, so only call it on paths where somebody is known to be at
// the console; unattended paths must not prompt at all.
...
void printCurrentConfig(const BridgeConfig &cfg) {
    ...
    printf("  Name:    %s\r\n", cfg.name);
    printf("  WiFi:    %s\r\n", cfg.wifiSsid[0] ? cfg.wifiSsid : "(not configured)");
...
void runSetupNow(BridgeConfig &cfg) {
    ...
    // WiFi SSID/password -- Enter alone keeps the saved value (so re-entering
    // setup just to change the role/name doesn't force retyping the WiFi
    // password every time). A blank SSID with nothing saved is re-prompted:
    // there is no fallback ("run without WiFi") for this build, unlike the
    // Ethernet target where a missing W5500 still leaves USB MIDI usable --
    // the whole point of this target is the network bridge.
    for (;;) {
        printf("WiFi SSID (Enter to use saved, or type new): ");
        char ssidAns[sizeof(cfg.wifiSsid)] = {};
        readLine(ssidAns, sizeof(ssidAns));
        if (ssidAns[0] == '\0') {
            if (cfg.wifiSsid[0] != '\0') break; // keep saved
            printf("No saved SSID -- WiFi SSID: ");
            continue;
        }
        strncpy(cfg.wifiSsid, ssidAns, sizeof(cfg.wifiSsid) - 1);
        cfg.wifiSsid[sizeof(cfg.wifiSsid) - 1] = '\0';
        break;
    }

    printf("WiFi password (Enter to use saved): ");
    char passAns[sizeof(cfg.wifiPassword)] = {};
    readLine(passAns, sizeof(passAns));
    if (passAns[0] != '\0') {
        strncpy(cfg.wifiPassword, passAns, sizeof(cfg.wifiPassword) - 1);
...
bool runClientHostSelect(BridgeConfig &cfg, IDiscovery &disc,
    ...
    // calling it before the netif has a valid IPv4 address burns the whole
    // browse window on a query that goes out with a bogus 0.0.0.0 source
    // and gets silently dropped. Wait briefly for a valid address first,
    // still pumping the network meanwhile.
    ...
    while (absolute_time_diff_us(get_absolute_time(), deadline) > 0) {
        // Pump cyw43_arch/lwIP's timers -- main()'s own loop that normally
        // does this hasn't started yet at this point in boot.
        pollNetwork();
        pumpTick();
    ...
    if (!manual && hostCount == 0) {
        ...
        // here would block in getchar() forever and main() would never get
        // to run the bridge at all. Give up instead; ESC re-enters setup later.
        if (!sawInput) {
    ...
    if (manual) {
        ...
        if (!sawInput) {
            ...
            // one, and prompting would block in readLine() forever. Take
