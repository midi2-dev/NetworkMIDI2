...
/*! \brief make the board's MAC address unique
 *  \ingroup w5x00_lwip
 *
 *  Overwrites the low 3 bytes of the global `mac[6]` (otherwise a fixed
 *  locally-administered default shared by every board built from this
 *  firmware) with bytes from this RP2040's factory-unique flash ID, so
 *  multiple NetworkMIDI2_Bridge boards on the same LAN don't collide (same
 *  MAC -> same DHCP lease/IP, ARP confusion). Call once, before
 *  wiznet_lwip_init()'s setSHAR(mac)/netif_add() -- changing `mac` after
 *  the netif is already using it has no effect.
 */
