...
// The actual role -> name -> DHCP/static setup walk that runConfigMenu()
// runs after its boot-time countdown. Exposed separately so ESC-while-
// running (main.cpp's kReconfigureKey handler) can jump straight into it
// without a countdown -- the user just pressed a key specifically to get
// here, so gating on "press any key within N seconds" again would be pure
// friction. Saves the result to flash before returning, same as
// runConfigMenu().
